...
echo "Requested version ${SWI_VERSION} of SWI-Prolog"
...
(
    ...
    (
        ...
        if [ ${SWI_BRANCH} == "master" ] ; then
            ...
            git clone --branch ${SWI_BRANCH} https://github.com/SWI-Prolog/swipl-devel.git
            cd swipl-devel
            ID=${SWI_VERSION}
        fi
        echo "----> Building SWI Prolog ${ID} in ${build}/swipl-build"
        # Prepare the build
        # SWI does not like to be compiled as root. What are containers for if you can't bash around as a superuser and ruin everything?
        # Make a tweak to remove this line
        sed -i -e "s@UID == 0@UID == 'garbage'@g" prepare
        # And go!
